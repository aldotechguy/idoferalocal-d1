...
export function loadOps(byCol: Map<string, DocRow[]>, stmts: Stmt[], ctx: Ctx) {
  ...
  for (const { d, o } of dedup('deliveryOrders', 'deliveryNo')) {
    stmts.push({ sql: `INSERT INTO delivery_orders (id, delivery_no, sale_id, invoice_no, customer_id, customer_name, delivery_address, items_json, delivery_fee_kobo, status, is_pickup_confirmed, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET status=excluded.status;`, params: [str(o.id || d.docId), str(o.deliveryNo || d.docId), o.saleId ? str(o.saleId) : null, str(o.invoiceNo), o.customerId ? str(o.customerId) : null, str(o.customerName), o.deliveryAddress ? str(o.deliveryAddress) : null, JSON.stringify(o.items || []), toKobo(o.deliveryFee), str(o.status, 'Pending Pickup'), bool01(o.isPickupConfirmed), str(o.createdBy), str(o.createdAt, nowIso())] });
  }
  for (const { d, o: w } of dedup('whatsAppPreOrders', 'preOrderNo')) {
    stmts.push({ sql: `INSERT INTO whatsapp_preorders (id, preorder_no, customer_id, customer_name, customer_phone, items_json, subtotal_kobo, total_kobo, status, converted_sale_id, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET status=excluded.status;`, params: [str(w.id || d.docId), str(w.preOrderNo || d.docId), w.customerId ? str(w.customerId) : null, str(w.customerName), str(w.customerPhone), JSON.stringify(w.items || []), toKobo(w.subtotal), toKobo(w.totalAmount), str(w.status, 'Pending Review'), w.convertedSaleId ? str(w.convertedSaleId) : null, str(w.createdBy), str(w.createdAt, nowIso())] });
  }
