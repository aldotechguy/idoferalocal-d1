...
interface Props {
  ...
  paid: number;
  setPaid: (v: number | undefined) => void;
  submitting: boolean;
  onSubmit: () => void;
}
...
export const CheckoutBody: React.FC<Props> = ({
  ...
  <>
    ...
    <PaymentField subtotal={subtotal} paid={paid} setPaid={setPaid} />
    <SubmitButton submitting={submitting} onSubmit={onSubmit} hasItems={items.length > 0} />
    <p className="text-[11px] text-slate-400 dark:text-slate-500 text-center">A receipt will be shown after payment.</p>
  </>
);
...
const PaymentField: React.FC<{ subtotal: number; paid: number; setPaid: (v: number | undefined) => void }> = ({
  subtotal,
  paid,
  setPaid,
}) => (
  <div>
    ...
    <input
      ...
      onChange={(e) => {
        ...
        setPaid(Number.isFinite(v) && v >= 0 ? v : undefined);
      }}
      className="w-full h-9 px-3 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-sm font-medium text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/50"
      placeholder="Same as subtotal"
    />
    <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-1">Leave blank to pay exact amount.</p>
    <div className="border-t border-slate-200 dark:border-slate-800 pt-3 space-y-1.5 text-sm mt-2">
      ...
      <div className="flex justify-between">
        <span className="text-slate-500 dark:text-slate-400">You pay</span>
        <span className="font-extrabold text-blue-600 dark:text-blue-400">{toNaira(paid)}</span>
      </div>
    </div>
  </div>
);
