...
type Draft = {
  publish: boolean; mallPrice: string; mallDescription: string;
...
const VIEWS = [
  ...
  { value: 'unlisted', label: 'Not published' },
  { value: 'blocked', label: 'Not ready to publish' },
];

export const MallListingsView: React.FC = () => {
  const { currentUser } = useAuth();
  const { showToast } = useToast();
  const canPublish = ['Administrator', 'Store Manager'].includes(currentUser?.role || '');

  const [listings, setListings] = React.useState<StaffMallListing[]>([]);
  const [counts, setCounts] = React.useState({ listed: 0, unlisted: 0 });
  ...
  const save = async () => {
    ...
    try {
      ...
      setBlockers(result.listing.blockPublish);
      await load();
      showToast({
        title: result.listing.listed ? 'Published to the Mall' : 'Removed from the Mall',
        message: result.listing.name, type: 'success',
      });
    } catch (err) {
      const failure = err as Error & { fields?: Record<string, string>; blockPublish?: string[] };
      setFieldErrors(failure.fields || {});
      if (failure.blockPublish) setBlockers(failure.blockPublish);
      showToast({ title: 'Listing not saved', message: failure.message, type: 'error' });
    } finally { setBusy(false); }
  };

  const notReady = listings.filter((listing) => listing.blockPublish.length).length;

  return <div className="space-y-4">
    <div className="grid sm:grid-cols-3 gap-3">
      <div className="rounded-2xl border bg-white dark:bg-slate-900 p-4"><p className="text-xs font-bold text-slate-500">Published</p><p className="text-2xl font-black">{counts.listed}</p></div>
      <div className="rounded-2xl border bg-white dark:bg-slate-900 p-4"><p className="text-xs font-bold text-slate-500">Not published</p><p className="text-2xl font-black">{counts.unlisted}</p></div>
      <div className="rounded-2xl border bg-white dark:bg-slate-900 p-4"><p className="text-xs font-bold text-slate-500">Not ready to publish</p><p className="text-2xl font-black">{notReady}</p></div>
    </div>
        ...
        {listings.map((listing) => <button type="button" key={listing.id} onClick={() => open(listing)} className="w-full p-4 text-left flex items-center justify-between gap-3 hover:bg-slate-50 dark:hover:bg-slate-800/60">
          <div className="min-w-0 flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              {listing.blockPublish.length > 0 && <p className="text-xs font-bold text-amber-600">Not ready: {listing.blockPublish[0]}</p>}
            </div>
          </div>
          <div className="text-right shrink-0">
            <p className="font-black text-sm">{money(listing.publicPriceKobo)}</p>
            <p className={`text-xs font-bold ${listing.listed ? 'text-emerald-600' : 'text-slate-400'}`}>{listing.listed ? 'Published' : 'Not published'}</p>
    ...
    <AccessibleOverlay open={Boolean(selected)} onClose={() => setSelected(null)} title={selected ? `Mall listing — ${selected.name}` : 'Mall listing'} description={selected ? `${selected.sku} · ${selected.productStatus} · stock ${selected.stock}` : undefined} className="max-w-2xl">
      {selected && draft && <div className="space-y-4">
        <label className="flex items-center gap-3 rounded-xl border p-3">
          <input type="checkbox" checked={draft.publish} disabled={!canPublish} onChange={(event) => setDraft({ ...draft, publish: event.target.checked })} />
          <span className="text-sm font-bold">Publish on the Mall storefront</span>
        </label>

        {blockers.length > 0 && <div className="rounded-xl border border-amber-300 bg-amber-50 dark:bg-amber-950/30 p-3">
          <p className="text-xs font-black text-amber-800 dark:text-amber-300">Fix these before publishing</p>
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          <label className="text-xs font-bold">Mall price (₦)
            <input inputMode="decimal" value={draft.mallPrice} disabled={!canPublish} onChange={(event) => setDraft({ ...draft, mallPrice: event.target.value })} placeholder={fromKobo(selected.retailPriceKobo)} className="mt-1 w-full h-10 px-3 rounded-xl border bg-transparent text-sm" />
          ...
          <label className="text-xs font-bold">Display order
            <input inputMode="numeric" value={draft.displayOrder} disabled={!canPublish} onChange={(event) => setDraft({ ...draft, displayOrder: event.target.value })} placeholder="Lower shows first" className="mt-1 w-full h-10 px-3 rounded-xl border bg-transparent text-sm" />
        ...
        <label className="text-xs font-bold block">Mall description
          <textarea value={draft.mallDescription} disabled={!canPublish} onChange={(event) => setDraft({ ...draft, mallDescription: event.target.value })} rows={4} placeholder={selected.internalDescription || 'Customer-facing description'} className="mt-1 w-full p-3 rounded-xl border bg-transparent text-sm" />
        ...
        <label className="flex items-center gap-3 rounded-xl border p-3">
          <input type="checkbox" checked={draft.featured} disabled={!canPublish} onChange={(event) => setDraft({ ...draft, featured: event.target.checked })} />
        ...
        <fieldset className="rounded-xl border p-3 space-y-3">
          ...
          <label className="text-xs font-bold block">Promotional price (₦)
            <input inputMode="decimal" value={draft.promoPrice} disabled={!canPublish} onChange={(event) => setDraft({ ...draft, promoPrice: event.target.value })} className="mt-1 w-full h-10 px-3 rounded-xl border bg-transparent text-sm" />
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <label className="text-xs font-bold">Starts
              <input type="datetime-local" value={draft.promoStart} disabled={!canPublish} onChange={(event) => setDraft({ ...draft, promoStart: event.target.value })} className="mt-1 w-full h-10 px-3 rounded-xl border bg-transparent text-sm" />
              {fieldErrors.promoStart && <span className="text-rose-600">{fieldErrors.promoStart}</span>}
            </label>
            <label className="text-xs font-bold">Ends
              <input type="datetime-local" value={draft.promoEnd} disabled={!canPublish} onChange={(event) => setDraft({ ...draft, promoEnd: event.target.value })} className="mt-1 w-full h-10 px-3 rounded-xl border bg-transparent text-sm" />
