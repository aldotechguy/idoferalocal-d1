  ...
  return <div className="space-y-4">
    ...
    <AccessibleOverlay open={Boolean(selected)} onClose={() => setSelected(null)} title={selected?.orderNo || 'Mall order'} description={selected ? `${selected.customerName} · ${selected.customerPhone}` : undefined} className="max-w-2xl">
      {selected && <div className="space-y-4">
        ...
        <div className="border-t pt-3 flex justify-between"><strong>Total</strong><strong>{money(selected.totalKobo)}</strong></div>
        {selected.delivery.address && <p className="text-sm text-slate-600 dark:text-slate-300"><strong>Delivery:</strong> {selected.delivery.address}</p>}
        {!selected.linkedSaleId && selected.status !== 'cancelled' && <div className="space-y-3 border-t pt-4">
          {selected.status === 'pending' && <button disabled={busy} onClick={() => act(() => staffMallClient.confirm(selected.id), 'Order confirmed')} className="w-full h-11 rounded-xl bg-blue-600 text-white font-bold inline-flex items-center justify-center gap-2"><CheckCircle2 className="w-4 h-4"/> Confirm Order</button>}
